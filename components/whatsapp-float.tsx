...
const floatingWhatsappHref = `https://wa.me/${site.whatsapp}?text=${encodeURIComponent(
  "Hi Dr. Sheetal, I have a query regarding homeopathic consultation..."
)}`;
...
export function WhatsAppFloat() {
  return (
    <motion.a
      ...
      aria-label="Chat on WhatsApp with Dr. Sheetal"
      initial={{ scale: 0, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      transition={{ delay: 0.8, duration: 0.4, type: "spring", stiffness: 260, damping: 20 }}
      whileHover={{ scale: 1.08 }}
      whileTap={{ scale: 0.94 }}
      className="fixed bottom-5 sm:bottom-6 right-4 sm:right-6 z-40 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_10px_30px_rgba(37,211,102,0.4)] transition-all duration-300 [transform:translate3d(0,0,0)]"
      style={{
